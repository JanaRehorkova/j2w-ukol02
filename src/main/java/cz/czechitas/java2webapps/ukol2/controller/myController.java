...
public class myController{
 ...
 private final ArrayList<String> texty = new ArrayList<>();
 String text1 = "Real programmers count from 0.";
 ...
 String text6 = "Programmer (noun.): A machine that turns coffee into code.";
    ...
    public ModelAndView citatdne(){
       ...
       texty.add(text5);
       texty.add(text6);
   int cisloObrazku = random.nextInt(6);
    String kodObrazku;
       kodObrazku = pozadi.get(cisloObrazku);


   int cisloCitatu=random.nextInt(6);
     String citatek= texty.get(cisloCitatu);

    ModelAndView result = new ModelAndView("citatdne");
       result.addObject("obrazek", String.format("https://source.unsplash.com/%s/1600x900", kodObrazku));
